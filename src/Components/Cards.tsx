...
export const TeamCard: React.FC<TeamCardProps> = ({
  ...
  return (
    <div className="w-full px-5 py-2 box-border bg-[#E8F4FD] flex items-start gap-6 rounded-lg">
      ...
      <p className="w-full h-full overflow-auto text-xs text-[#0E1629] pt-8 leading-5">{about}</p>
    </div>
  );
};
...
const NewsCards: React.FC<NewsCardProps> = ({
  ...
  return (
    <section
      className={`min-w-[50%] h-full max-h-[200px] overflow-auto w-1/2 flex items-start gap-2 p-5 box-border ${
    ...
    >
      <span
        ...
        } rounded-full p-2 text-2xl text-white`}
      >
